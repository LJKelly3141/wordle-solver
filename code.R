...
pacman::p_load('readxl','stringr','tidyverse','svDialogs')
...
guess.eval <- function(words, guess, feedback) {
      ...
      mutate(
        "filter{n}" := case_when(
          feedback[n] == 1 ~
            .[n + 1] == word.split[n],
          feedback[n] == 0 ~
           !str_detect(.$word, word.split[n]),
          feedback[n] == -1 ~
            str_detect(.$word, word.split[n]) & .[n + 2] != word.split[n],
        )
      )
  }
  word.filter <- words%>%
...
word.list <- make.word.list(df=scrable.list)
words <- word.list$`Word List` 
guess <- word.list$`Guess S2`

df <- guess.eval(words,guess,c(1,0,0,-1,0))
...
df <- guess.eval(words,guess,feedback)
word.list <- make.word.list(df=df)
view(word.list$"Word List")
words <- word.list$`Word List`
guess <- word.list$`Guess S2`
if(nrow(words)<=1){g=10}
}
